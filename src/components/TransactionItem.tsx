...
export default function TransactionItem({ txn, category, wallet, onEdit, onDelete }: Props) {
  ...
  return (
    <div
      className="flex items-center gap-3 surface-card p-3 card-interactive"
      ...
      {showActions ? (
        <div className="flex items-center gap-1 shrink-0">
          <button onClick={e => { e.stopPropagation(); onEdit(txn); setShowActions(false); }}
            ...
            className="w-9 h-9 rounded-lg bg-primary/10 flex items-center justify-center transition-all active:scale-90">
          ...
          <button onClick={e => { e.stopPropagation(); onDelete(txn); setShowActions(false); }}
            ...
            className="w-9 h-9 rounded-lg bg-destructive/10 dark:!bg-red-400/10 flex items-center justify-center transition-all active:scale-90">
        ...
        <p className={`text-sm font-semibold shrink-0 tabular-nums ${
          ...
          txn.type === 'transfer' ? 'text-primary' : 'text-destructive'
        }`}>
